...
import bluetooth
# Remove BLE imports since they're handled in boot.py
...
# Note: BLE communication is now handled by InitService in boot.py
# The on_rx callback and sp.send() calls are no longer needed here
...
#             led.value(0)
        if (time.ticks_ms() - debounce_time) > 300:
            # The following lines are no longer needed as BLE communication is handled by InitService
